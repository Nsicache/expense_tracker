export class BudgetDashboard {
      ...
      <div class="category-item" data-category="${category.name}">
        <div>
          ...
          <div class="category-meta">
            Assigned: ₦${this.format(category.assigned)} | 
            Spent: ₦${this.format(category.spent)} | 
  ...
  handleAssign(categoryName) {
    ...
    const amount = parseFloat(prompt(
      `How much to assign to ${categoryName}?\n(Available: ₦${this.format(unassigned)})`
    ));
    
    if (!isNaN(amount) && amount > 0) {
      if (amount > unassigned) {
        alert(`Cannot assign more than available (₦${this.format(unassigned)})`);
        return;
      }
      ...
      this.app.updateState({ categories: updated });
    }
  }
  ...
  showAddCategoryForm() {
    this.container.insertAdjacentHTML('beforeend', `
      <div class="card" id="add-category-form">
        ...
        <div class="form-group">
          <input type="number" id="new-category-amount" 
                 placeholder="Initial Amount" min="0" step="100">
        </div>
